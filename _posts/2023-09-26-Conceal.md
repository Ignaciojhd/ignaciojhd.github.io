...
Nice! finally we can se some TCP ports. Let's run some common nmap scripts on this ports:
...
The victims has FTP with anonymous login enable, this means that we can connect to the FTP service using "anonymous" as the user and an empty password. In this case there are no files but we do have permission to upload files. 
...
This privilege can be easily exploite with [JuicyPotato](https://github.com/ohpe/juicy-potato) . We just need to check which Windows version is running to specify a correct [CLSID](https://github.com/ohpe/juicy-potato/blob/master/CLSID/README.md) 
